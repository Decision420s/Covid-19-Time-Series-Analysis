...
$$
\begin{aligned}
log(\text{Covid}_t) &= \beta_0 + \beta_1 t + \beta_2 t^2 + \beta_3 cos(\frac{2 \pi 5 t}{60}) + \beta_4 sin(\frac{2 \pi 5 t}{60}) + \beta_5 cos(\frac{2 \pi 8 t}{60}) \\
&+ \beta_6 cos(\frac{2 \pi 8 t}{60}) + \beta_7 cos(\frac{2 \pi 9 t}{60}) + \beta_8 cos(\frac{2 \pi 9 t}{60}) + \beta_9 cos(\frac{2 \pi 17 t}{60}) + \beta_{10} cos(\frac{2 \pi 17 t}{60}) + X_t \notag
(\#eq:param-equation)
\end{aligned}
$$

Figure \ref{fig:signal-1} presents relevant plots including Spectral density of transformed data, the fitted model, residuals, and the Spectral density of residuals.

```{r signal-1, echo=FALSE, fig.cap='Spectral Density of the log transform of Covid data, Fitted Model, Residuals, and Spectral Density of Residuals', fig.align='center', fig.pos="H", out.width="80%"}
...
```{r acf-pacf-signal-1, echo=FALSE, results="hide", fig.cap="Autocorrelation function (ACF) and partial autocorrelation function (PACF) values for the parametric signal model's residuals. Blue circles reflect the ARMA(0,0) model, while the orange circles reflect the ARMA(0,1) model.", fig.align = 'center', fig.pos="H", fig.height=4, fig.width = 8, out.width="80%"}
layout(matrix(c(1,2),ncol=2))
lags <- 25
...
```{r estimatetable}
x <- kable(model2_1$ttable[,1:2], caption = 'Model Estimates')
kable_styling(x, position = 'center', latex_options = 'HOLD_position')
```
## Prediction
Figure \ref{fig:final-pred} shows the forecast values of Covid new cases in next 10 days.
