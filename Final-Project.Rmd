...
## Differencing
In addition to fitting a parametric model on the signal, differencing is also considered. The periodogram of the log transformed data shows seasonal components at frequencies of 8/60 and 9/60, which correspond to periods at 60/8=7.5 and 60/9=6.67. This might occur due to leakage where the true period is 7, which is essentially weekly effect and makes more sense. Furthermore, to remove the quadratic trend in the tranformed data, a second differencing with lag=1 is applied after the first differencing at lag=7.
```{r signal-2, echo=FALSE, results='hide', fig.cap='Periodogram of $log(\\text{COVID}_t)$ and Residuals after differnecing. $log(\\text{COVID}_t)$ is the logarithemic transform of the COVID data.',  fig.align = 'center', fig.pos="H", fig.height=3.5, fig.width = 8, out.width="80%"}
layout(matrix(c(1,2),ncol=2))
